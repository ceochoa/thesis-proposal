...
much better results for \texttt{Range Minimum Query} indexes given that,
from the point of view of \texttt{Range Minimum Query} indexes,
...
We describe next two problems in Computational Geometry for which we
plan to develop synergistic solutions. 
...
Given two points $p = (a,b)$ and $q = (x,y)$ in the plane, $p$
...
two dimensions and online \texttt{domination} queries on sets of
...
\textbf{\begin{enumerate}
  ...
  \item Is there an algorithm that takes advantage of the input order for
    computing the \textsc{Maxima} in a more efficient way
    than the previous described?
    %% Synergistic Maxima
  \item Is there an algorithm for computing the \textsc{Maxima} which takes the
  ...
  \item Is there a {\textsc{MultiMaxima}} algorithm and a deferred data structure
    for answering \texttt{domination} queries, which
...
\textbf{
  ...
  \item Are there algorithms for computing the \textsc{Convex Hull}
    that take advantage of the input order and that consider other
    measures of difficulty other than the one described previously?
  \item Is there an algorithm for computing the \textsc{Convex Hull}
    of a set of points, which takes the best advantage of both the
    \emph{input order} and the \emph{input structure} in a synergistic
    way, so that it performs as good as previously known solutions on
    all instances, and much better on instances where it can take
  ...
  \item Is there a \textsc{MultiConvexHull} algorithm and a deferred
    data structure for answering \texttt{convex intersection} queries which
...
Next, we conclude with a review of the results that we have already
submitted to an international peer reviewed conference, and those
problems on which we are currently working. For those problems, we describe
some preliminary results and explain the plan for developing them.
...
The results about \textsc{Sorting} solutions described in
...
Aspects of Computer Science (STACS 2017) and already received positive
reviews (see the appendix) during the rebuttal period. The article was
submitted by the author and his advisor in collaboration with
Srinivasa Rao Satti from the Department of Computer Science and
Engineering, Seoul National University, South Korea. The author final
notification is scheduled for the 12th of December, 2016.

% Maxima

\subsection{November 2016 - February 2017: Synergistic Algorithms and
